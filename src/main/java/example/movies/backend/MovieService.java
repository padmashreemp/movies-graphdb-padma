...
public class MovieService {
    ...
    public Integer addMovie(String moviename,String tagline,String movieyear,String actorname,String actoryear,String directorname,String directoryear) {
        try (Session session = getSession()) {
            return session.writeTransaction( tx -> {
                
                Result result = tx.run(
                        "CREATE ($moviename:Movie {title: $moviename,released: $movieyear,tagline: $tagline})");
                result = tx.run(
                        "CREATE ($actorname:Person {name:$actorname, born:$actoryear})");
                result = tx.run(
                        "CREATE ($directorname:Person {name:$directorname, born:$directoryear})");

                result = tx.run(
                        "CREATE($actorname)-[:ACTED_IN ]->($moviename),($directorname)-[:DIRECTED]->($moviename) ");
